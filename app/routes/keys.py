...
def validate_key(request: schemas.KeyValidate, req: Request, db: AsyncSession = Depends(get_db)):
    logger.debug("Starting validate_key")
    client_ip = req.client.host
    logger.debug(f"Client IP: {client_ip}")
    
    if crud.is_blacklisted(db, request.key):
    ...
    was_blacklisted, reason = crud.auto_blacklist_check(db, request.key, request.hwid, client_ip)
    ...
    if was_blacklisted:
        ...
        # Get the key ID
        db_key = (db.execute(select(Key).filter_by(key=request.key))).scalars().first()
        if not db_key:
            raise HTTPException(status_code=404, detail="Key not found")
        
        # Count failed attempts
        logs = (db.execute(select(Log).filter_by(key_id=db_key.id))).scalars().all()
        failed_count = len([log for log in logs if not log.success]) + 1  # +1 for this attempt
        if failed_count <= 3:
    ...
    db_key = crud.increment_key_use(db, request.key, request.hwid, client_ip)
    if not db_key:
        raise HTTPException(status_code=404, detail="Key not found")
...
def create_key(key: schemas.KeyCreate, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Not authorized")
    return crud.create_key(db, key)

@router.get("/")
def get_all_keys(db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Not authorized")
    return crud.get_all_keys(db)

@router.delete("/{key}")
def delete_key(key: str, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    if not key:
...
def reset_hwid(key: str, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    if not key:
        raise HTTPException(status_code=404, detail="Key cannot be empty")
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Not authorized")
    # Clear HWID logs
    db.execute("DELETE FROM logs WHERE key_id = (SELECT id FROM keys WHERE key = :key)", {"key": key})
    db.commit()
